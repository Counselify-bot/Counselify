    ...
    return (
        <div className="pt-24 pb-32 bg-surface-container-low min-h-screen text-left">
            <div className="container mx-auto px-6 max-w-5xl">
                ...
                <div className="mb-16">
                    <span className="text-[11px] font-bold uppercase tracking-[0.4em] text-error">Video Guides</span>
                    <h1 className="text-5xl md:text-6xl font-bold text-slate-900 mt-4 serif-font italic tracking-tighter">YouTube Resources</h1>
                </div>

                <div className="grid grid-cols-1 gap-8">
                    {videos.map((vid, idx) => (
                        <div key={idx} className="bg-white p-8 rounded-[40px] shadow-xl border border-outline-variant/20 flex flex-col md:flex-row items-center justify-between gap-8 group hover:border-red-500 transition-all">
                            <div className="flex items-center gap-8">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-bold text-on-surface">{vid.title}</h3>
                                    <div className="flex items-center gap-4 text-[10px] font-bold text-outline uppercase tracking-widest">
                                        <span>{vid.duration}</span>
                                        <div className="w-1 h-1 bg-slate-200 rounded-full"></div>
                                        <span>{vid.views} Views</span>
                                    </div>
                                    <p className="text-on-surface-variant font-bold italic text-sm">{vid.desc}</p>
                                </div>
                            </div>
                            <button className="px-8 py-5 bg-red-600 text-white rounded-[24px] font-bold text-[11px] uppercase tracking-widest shadow-xl hover:bg-red-700 transition-all flex items-center gap-3 active:scale-95">
