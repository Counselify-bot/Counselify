    ...
    return (
        <div className="pt-24 pb-32 bg-slate-50 min-h-screen text-left">
            <div className="container mx-auto px-6 max-w-5xl">
                ...
                <div className="mb-16">
                    <span className="text-[11px] font-black uppercase tracking-[0.4em] text-orange-500">Preparation kit</span>
                    <h1 className="text-5xl md:text-6xl font-black text-slate-900 mt-4 serif-font italic tracking-tighter">Essential Checklists</h1>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {lists.map((list, idx) => (
                        <div key={idx} className="bg-white p-10 rounded-[48px] shadow-xl border border-slate-100 group hover:border-orange-500 transition-all flex flex-col justify-between">
                            <div>
                                <div className="w-16 h-16 bg-orange-50 rounded-2xl flex items-center justify-center text-orange-500 mb-8">
                                    <FileBox size={32} />
                                </div>
                                <h3 className="text-2xl font-black text-slate-800 mb-4">{list.title}</h3>
                                <p className="text-slate-500 font-bold italic text-sm mb-6">{list.desc}</p>
                            </div>
                            <div className="flex items-center justify-between mt-auto">
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{list.items} Required Items</span>
